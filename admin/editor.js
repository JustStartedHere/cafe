...
import { mutators, InvalidMenuError } from './menu-model.js';
...
export function createEditor({ store, onAuthError }) {
  ...
  /**
   * Satu-satunya jalan menulis. `mutate` adalah fungsi `(menu) => menu` —
   * store yang mengurus retry saat 409, dengan menerapkan ulang fungsi ini.
   */
  async function commit(mutate, message, { onSuccess } = {}) {
  ...
  itemList.addEventListener('click', async (event) => {
    ...
    switch (action) {
      ...
      case 'item-delete':
        ...
        await commit(mutators.removeItem(id), `Hapus item ${item.name.id}`);
        break;
      default:
        break;
    }
  ...
  itemForm.addEventListener('submit', async (event) => {
    ...
    try {
      const mutate = id ? mutators.updateItem(id, draft) : mutators.addItem(draft);
      const label = draft.name.id.trim() || 'item';
      // Form baru ditutup setelah PUT sukses. Gagal jaringan → isian tetap utuh.
      await commit(mutate, id ? `Ubah item ${label}` : `Tambah item ${label}`, { onSuccess: closeForm });
    } catch (error) {
      // Mutator memvalidasi saat dipanggil; tangkap di sini agar form tidak tertutup.
      setError(error);
    }
  });
